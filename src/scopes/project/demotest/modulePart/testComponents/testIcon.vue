<template>
    <div>
        <ta-icon type="step-backward"/>

        <h4>基本用法</h4>
        <span>使用 &lt;Icon/&gt; 标签声明组件，指定图标对应的 type 属性。<br>
            可以通过 theme 属性来设置不同的主题风格的图标，也可以通过设置 spin 属性来实现动画旋转效果。</span>

        <div class="icons-list">
            <ta-icon type="home"/>
            <ta-icon type="setting" theme="filled"/>
            <ta-icon type="smile" theme="outlined"/>
            <ta-icon type="sync" :spin="true"/>
            <ta-icon type="loading"/>
        </div>
        <br>
        ...
        <span>对于使用 iconfont.cn 的用户，通过设置 createFromIconfontCN 方法参数对象中的 scriptUrl 字段， 即可轻松地使用已有项目中的图标。</span>
        ...
        <h4>多色图标</h4>
        <span>可以通过设置 theme 属性为 twoTone 来渲染双色图标，并且可以设置主题色。</span>
        <div class="icons-list">
            <ta-icon type="smile" theme="twoTone"/>
            <ta-icon type="heart" theme="twoTone" twoToneColor="#eb2f96"/>
            <ta-icon type="check-circle" theme="twoTone" twoToneColor="#52c41a"/>
        </div>
    </div>
</template>
